...
ngx_http_vod_map_media_set_state_machine(ngx_http_vod_ctx_t *ctx)
{
	...
	{
		ngx_log_debug0(NGX_LOG_DEBUG_HTTP, ctx->submodule_context.request_context.log, 0, "MAP RUN STEP");
		rc = ngx_http_vod_map_run_step(ctx);
		if (rc != NGX_OK)
		{
			return rc;
	...
	ngx_log_debug0(NGX_LOG_DEBUG_HTTP, ctx->submodule_context.request_context.log, 0, "DYNAMIC CLIP START");
...
ngx_http_vod_mapped_request_handler(ngx_http_request_t *r)
{
	...
	{
		ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "no upstream location");
	...
	{
		ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "has upstream location");
	...
	const ngx_http_vod_request_t** request)
{
	...
	rc = ngx_http_vod_parse_uri_path(
		...
		conf->try_suburis_middles,
		&conf->try_suburis_postfix,
